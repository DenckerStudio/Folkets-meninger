...
- **n8n MCP (workflow deploy):** Cursor MCP must point at `https://n8n.heyklever.app/mcp-server/http` (not `n8n.dencker.no`). Generate a fresh token in n8n → Settings → Instance-level MCP → API key, and add `N8N_MCP_TOKEN` as a Cloud Agent secret when MCP tools are unavailable in the VM. Deploy: `npx tsx scripts/export-n8n-workflows.mjs` then `N8N_MCP_TOKEN=... node scripts/deploy-n8n-workflows-mcp.mjs --publish`. Active workflows use Supabase node + credential «Folkets-meninger»; run `supabase db push` for `20260823140000_n8n_supabase_rpc.sql` first.
