...
import { SakPageTabs } from '@/components/sak/sak-page-tabs';
...
export default async function SakPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const officialTitle = (detailedContent?.korttittel || detailedContent?.tittel || sak.title).trim();
  const officialDescription = (detailedContent?.tittel || sak.summary || '').trim();
    ...
    <div className="max-w-4xl mx-auto space-y-8 pb-16 sm:space-y-10 sm:pb-12">
      <FadeIn delay={0.1}>
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <SakPageActions sakId={sak.id} title={officialTitle} className="sm:justify-end" />
        </div>
      </FadeIn>
          ...
          <div className="space-y-6">
            ...
            {parentestekst ? (
              <div className="rounded-xl border border-border bg-muted/40 p-4">
                <p className="text-sm text-muted-foreground">{parentestekst}</p>
              </div>
            ) : null}

            {(innstillingstekst || kortvedtak || vedtakstekst) ? (
              <div className="space-y-6 rounded-2xl border border-border bg-card p-6 shadow-sm">
                <h2 className="mt-0 text-xl font-bold text-foreground">Sakens detaljerte innhold</h2>
